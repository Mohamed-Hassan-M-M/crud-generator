...
class CrudCommand extends Command
{
    ...
    protected $route = '';//edit mhmm

    protected $name = '';//edit mhmm
    ...
    public function handle()
    {
        ...
        foreach ($fieldsArray as $item) {
            ...
            if(in_array('trans=1', $spareParts)){
                $translatableArray[] = $spareParts[0];
            }
        ...
        $commaSeparetedString = implode("', '", $translatableArray);
        $translatable = "['" . $commaSeparetedString . "']";
        $languages = \App\Models\Language::select('code')->pluck('code')->toArray();
        $translatableWithCode = [];
        foreach ($translatableArray as $transArray) {
            foreach ($languages as $language){
                $translatableWithCode[] = $transArray . '_' . $language;
            }
        }
        $commaSeparetedString = implode("', '", $translatableWithCode);
        $translatableWithCode = "['" . $commaSeparetedString . "']";
        ...
        //edit mhmm
        $this->call('crud:controller', ['name' => $controllerNamespace . $name . 'Controller', '--crud-name' => $name, '--model-name' => $modelName, '--model-namespace' => $modelNamespace, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--pagination' => $perPage, '--fields' => $fields, '--validations' => $validations, '--translatable' => $translatable, '--translatableArray' => $translatableArray, '--translatableWithCode' => $translatableWithCode]);
        $this->call('crud:model', ['name' => $modelNamespace . $modelName, '--fillable' => $fillable, '--table' => $tableName, '--pk' => $primaryKey, '--relationships' => $relationships, '--soft-deletes' => $softDeletes, '--translatable' => $translatable]);
        $this->call('crud:migration', ['name' => $migrationName, '--schema' => $this->option('fields'), '--pk' => $primaryKey, '--indexes' => $indexes, '--foreign-keys' => $foreignKeys, '--soft-deletes' => $softDeletes]);
        $this->call('crud:view', ['name' => $name, '--fields' => $fields, '--validations' => $validations, '--view-path' => $viewPath, '--route-group' => $routeGroup, '--localize' => $localize, '--pk' => $primaryKey, '--form-helper' => $formHelper, '--translatable' => $translatable, '--translatableArray' => $translatableArray]);
        if ($localize == 'yes') {
    ...
    {
        return ["//$this->route routes". "\n" ."Route::delete('" . $this->route . "/bulk_delete', '" . $this->name . "Controller@bulkDelete')->name('" . $this->route . ".bulk_delete');" . "\n" . "Route::resource('" . $this->route . "', '" . $this->name . "Controller');"];//edit mhmm
    }
